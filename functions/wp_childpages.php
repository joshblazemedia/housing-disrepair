   ...
   $children = get_pages( array( 
    ...
    foreach( $children as $childpage ) 
    {
        ...
        $link = get_permalink( $childpage->ID );
          ...
          $return .= '<div class="overflow-hidden position-relative col-xl-6 col-lg-6 col-md-12 col-12 mb-3" style="min-height: 300px;">';
    
   $return .='<div class="h-100 overflow-hidden opacity-gradient-dark z-index-100">'
       .'<a href="' . $link . '">'
   .'<div class="w-100 h-100 opacity-gradient-dark z-index-100">'
   .'</div>'
    .'<div class="bgImage bg-secondary top-0 fade-in position-absolute m-0" data-trigger="titleTrigger" style="background: url('.$image[0].'); background-repeat: no-repeat;
  background-size: cover; background-position: center; filter: brightness(1) opacity(1); height: 110%; width: 110%;">'
   .'</div>'
. '<div class="col-12 bg-light text-center bottom-0 position-absolute mx-auto z-index-100  text-light p-3">'
.'<h6 class="text-text">'.$childpage->post_title.'</h6>'
